...
-- ---- 2. テーブルの作成 ----
create table department(id serial, name varchar(100) not null, PRIMARY KEY (id));
create table employee(id serial, department_id integer, name varchar(100) not null, name_kana varchar(100) not null, PRIMARY KEY (id), FOREIGN KEY (department_id) REFERENCES department(id));
create table employee_account(id serial, employee_id integer, name varchar(20) not null, password varchar(200), failed_attempts integer default 0 not null, lock_time timestamp null, primary key (id), FOREIGN KEY (employee_id) REFERENCES employee(id));
create table customer(id serial, name varchar(20) not null, name_kana varchar(20) not null, address1 varchar(100), address2 varchar(100), phone_number varchar(20), mail_address varchar(200), username varchar(30) not null, password varchar(200), register_date timestamp, PRIMARY KEY (id));
create table order_status(id serial, name varchar(100) not null, PRIMARY KEY (id));
create table payment_method(id serial, name varchar(100) not null, PRIMARY KEY(id));
create table orders(id serial, customer_id integer, order_status_id integer, payment_method_id integer, order_date timestamp, amount_total integer, PRIMARY KEY(id), FOREIGN KEY (customer_id) REFERENCES customer(id), FOREIGN KEY (order_status_id) REFERENCES order_status(id), FOREIGN KEY (payment_method_id) REFERENCES payment_method(id));
create table product_category(id serial, name varchar(20) not null, PRIMARY KEY(id));
create table product(id serial, product_category_id integer, name varchar(20) not null, price integer, image_url varchar(200), delete_flag integer, PRIMARY KEY(id), FOREIGN KEY(product_category_id) REFERENCES product_category(id));
create table product_stock(id serial, product_id integer, quantity integer, PRIMARY KEY (id), FOREIGN KEY (product_id) REFERENCES product(id));
create table order_detail(id serial, order_id integer, product_id integer, customer_id integer, count integer, PRIMARY KEY (id), FOREIGN KEY (order_id) REFERENCES orders (id), FOREIGN KEY (product_id) REFERENCES product (id), FOREIGN KEY (customer_id) REFERENCES customer (id));
...
(1, '赤ボールペン', 120, 'images/red_pen.jpg', 0),          -- ★追加 (20件目)
(1, '赤ボールペン（太字）', 150, 'images/red_pen_o.jpg', 0), -- ★追加 (21件目)
